<div class="sf_admin_list">
    <table class="datatable table table-bordered table-striped" id="table_csdl_hoi_vien_cho_duyet" style="margin-top: 5px !important;">
      <thead>
          <tr>
          ...
          <?php include_partial('csdlHoiVienChoDuyet/list_th_tabular', array('sort' => $sort)) ?>
      ...
      <!--   End thongnq1   -->
        <?php foreach ($results as $i => $csdl_lylichhoivien): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>

          <tr class="sf_admin_row <?php echo $odd ?> {pk: <?php echo $csdl_lylichhoivien->getId() ?>}" rel="<?php echo $csdl_lylichhoivien->getId() ?>">

                            <?php include_partial('csdlHoiVienChoDuyet/list_td_batch_actions', array('csdl_lylichhoivien' => $csdl_lylichhoivien, 'helper' => $helper)) ?>
                            <?php $orderNo = ($i) + ($currentPage - 1)*$pager->getMaxPerPage(); ?>
                <?php include_partial('csdlHoiVienChoDuyet/list_td_tabular', array('csdl_lylichhoivien' => $csdl_lylichhoivien, 'orderNo' => $orderNo)) ?>
                            <?php include_partial('csdlHoiVienChoDuyet/list_td_actions', array('csdl_lylichhoivien' => $csdl_lylichhoivien, 'helper' => $helper)) ?>
                      </tr>
        <?php endforeach; ?>
      </tbody>
      <tfoot>
        <tr>
          <th colspan="7">
            ...
            <div style="position: relative; width: auto; float:right"><?php include_partial('csdlHoiVienChoDuyet/pagination', array('pager' => $pager)) ?></div>
